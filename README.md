...
> ### Notes
- When unexpected behavior or values are observed during the experiment, allowing the program to immediately decide the next experimental conditions could result in proceeding with incorrect parameters. By having a human step in to review the results partway through and determine whether any abnormalities are present or whether the experiment should continue, we enhance both the safety and the reliability of the procedure. For example, since the fabricated crystals are small and fragile, they may break during the experiment. If this occurs, the experiment must be repeated. Due to the difficulty in handling the samples, appropriate judgment by the experimenter is essential. Therefore, steps requiring human clicks and inputs were introduced.
